...
| PC-D-3 | Drawer-storage rotation panic on `current_drawer.checked_add(1).expect("drawer overflow")` in `alloc_slot_and_rotate_if_needed`. | Low | Low | Triggered only after `u32::MAX` drawer rotations × 524,288 slots ≈ 2.25 quadrillion UTXO creates. Practically unreachable. | Low |
