...
| Asset SAC | High | Trusted to enforce its own transfer semantics. The Privacy Channel does not validate the asset contract beyond storing its address; it assumes a well-behaved SAC. In particular, a channel asset **must require `from`'s authorization to transfer**: the depositor's consent to the deposit *amount* is enforced by the asset's `transfer(from, channel, amount)` requiring `from`'s auth, not by the channel (the channel requires `from` to authorize only the condition list). An asset that can move value without `from`'s authorization must not be used as a channel asset — with such an asset, nothing binds a signed condition list to the amount it is paired with. |
...
- **PC-12 (overflow safety on totals).** Summing `total_deposit` and `total_withdraw` uses `checked_add`; an overflow errors `AmountOverflow`. The internal supply uses `checked_add` / `checked_sub` via `treasury.rs`, surfacing `AmountOverflow` or `AmountUnderflow` through the central error module. *Enforced by `pre_process_channel_operation` and `treasury.rs`.*
- **PC-13 (depositor consent).** Every `ExtDeposit` requires `from.require_auth_for_args(vec![&e, conditions])`. The depositor cannot have funds pulled from their account without explicitly signing for the exact condition list. Consent to the deposit *amount* is enforced one layer down, by the asset SAC's `transfer` requiring `from`'s authorization for the exact `(from, to, amount)` triple — see the Asset SAC trust assumption in §3.6. *Enforced by `transact.rs:122`.*
...
Known constraint on PC-16 (RV audit A3, accepted): conditions can deliberately repeat and overlap across multiple operations — this helps obfuscate which condition belongs to which signer and opens up different composition strategies. The flip side is that the binding compares conditions as a **set keyed by canonical XDR bytes**, so byte-identical conditions collapse to one entry that the contract considers one single operation, discharged by a single executed occurrence (e.g. two conditions each for a withdraw of 10 tokens to address A enforce one single withdraw of 10) — while PC-4 still counts every spent/deposited amount in full. If two independently signed spends each demand the identical `ExtWithdraw(to, amount)`, a bundle containing both pays `to` once and the difference becomes unsigned residual the composer allocates. This is deliberately **not** enforced in-contract; it is a trust-boundary constraint on the composer (already a semi-trusted role — see PC-14 and §5):
...
- **PC-17 (well-formed UTXO key encoding — audit A2).** No UTXO may be created under a key that is not a 65-byte SEC1 *uncompressed* P-256 point encoding (leading marker byte `0x04`); such a create errors `InvalidUtxoKey`. The key is the spending credential (`__check_auth` releases a UTXO only on a `secp256r1_verify` under it, which traps on a malformed key), so a UTXO funded under a malformed key would be permanently unspendable. This is a create-time marker-byte guard only — no per-spend cost. *Enforced by `Store::create` in `modules/storage/src/lib.rs` (every create path funnels through it).* **Limitation:** this is the marker-byte minimum, not full on-curve validation — soroban-sdk 25.3.0 exposes no P-256 point-validation host primitive, and full on-curve checking would require in-contract 256-bit field arithmetic paid on every create. A `0x04`-tagged but off-curve key still passes and would lock on spend, so **wallets must validate that a key is a valid on-curve P-256 point before funding it**; possession/encoding errors are undetectable on-chain regardless.
...
- **Stellar Asset Contract** behavior: the channel trusts its configured asset SAC to enforce its own transfer semantics — including that `transfer` requires `from`'s authorization, which is what binds a depositor's consent to the deposit amount (§3.6). Custom non-SAC assets are not a tested path, and an asset that can move value without `from`'s authorization must not be used as a channel asset.
