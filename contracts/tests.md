...
#### `tests/tests.rs::test_admin_transfer_keeps_current_admin_in_control_until_acceptance`

Regression test for the OpenZeppelin Ownable two-step admin transfer.

- Starts an admin transfer with `set_admin`.
- Asserts `admin()` still returns the original admin before acceptance.
- Asserts the original admin can still perform an owner-gated provider add.
- Asserts the pending admin cannot perform owner-gated provider changes until
  `accept_admin` completes.

#### `tests/tests.rs::test_admin_transfer_requires_pending_admin_to_accept`

Negative and positive ownership-transfer coverage.

- Starts an admin transfer.
- Asserts a non-pending address cannot call `accept_admin`.
- Asserts the pending admin can call `accept_admin`.
- Asserts the old admin loses provider-management privileges after acceptance.
- Asserts the new admin gains provider-management privileges after acceptance.
...
#### `test/test.rs::test_admin_transfer_keeps_current_admin_until_acceptance`

Regression test for the Privacy Channel's OpenZeppelin Ownable two-step admin
transfer wrapper.

- Starts an admin transfer with `set_admin`.
- Asserts `admin()` remains the original admin before acceptance.
- Asserts the pending admin cannot perform a further owner-gated `set_admin`
  call before accepting ownership.

#### `test/test.rs::test_admin_transfer_requires_pending_admin_to_accept`
...
- Asserts a non-pending address cannot call `accept_admin`.
- Asserts the pending admin can call `accept_admin`.
- Asserts the old admin cannot start a new admin transfer after acceptance.
- Asserts the new admin can start a subsequent admin transfer after acceptance.
...
- The storage drawer rotation path is covered by `modules/storage/src/test.rs::rotates_to_the_next_drawer_when_the_current_drawer_is_full`, but the coverage is storage-level rather than full-contract E2E. That is intentional because a full contract test would need to allocate 524,288 UTXOs to cross the drawer boundary.
