...
use crate::core::OperationBundle;
...
impl UTXOOperationBuilder {
    ...
    pub fn get_operation_bundle(&self) -> OperationBundle {
        OperationBundle {
    ...
    pub fn add_spend(&mut self, utxo: BytesN<65>, conditions: Vec<Condition>) {
        for (existing_utxo, existing_conditions) in self.spend.iter() {
            ...
            Self::verify_no_conflicting_conditions(conditions.clone(), existing_conditions.clone());
        }
    ...
    pub fn add_create(&mut self, utxo: BytesN<65>, amount: i128) {
        ...
        for (_, existing_conditions) in self.spend.iter() {
            Self::verify_condition_does_not_conflict_with_set(
                condition.clone(),
    ...
    pub fn add_provider_signature(
        ...
        let provider_bytes = address_to_ed25519_pk_bytes(&e, &provider_address)
            .unwrap_or_else(|_| panic!("address to pk"));

        self.sign_map.set(
            SignerKey::Provider(provider_bytes),
            (Signature::Ed25519(signature), 0), // live_until_ledger not needed for provider signatures as they sign the entire payload
        );
    }
    ...
    fn verify_condition_does_not_conflict_with_set(
        condition: Condition,
    ...
    fn verify_no_conflicting_conditions(
