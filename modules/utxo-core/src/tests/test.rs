use moonlight_auth::core::Condition;
...
use crate::{
    core::{Error as ContractError, OperationBundle},
    testutils::{
        contract::create_contract_with_mocked_auth, operation_bundle::calculate_auth_requirements,
    },
};
...
fn test_transfer() {
    ...
    let repeating_create_op = OperationBundle {
        create: vec![
            &e,
    ...
    let repeating_spend_op = OperationBundle {
        create: vec![
            &e,
            (utxo_d.public_key.clone(), 200_i128),
    ...
    let unbalanced_op = OperationBundle {
        create: vec![
            &e,
            (utxo_d.public_key.clone(), 200_i128),
    ...
    let balanced_op = OperationBundle {
        create: vec![
            &e,
            (utxo_d.public_key.clone(), 200_i128),
            (utxo_e.public_key.clone(), 200_i128),
...
fn test_transfer_auth_mocked() {
    ...
    let auth_req = calculate_auth_requirements(&e, &op.spend);
...
fn test_transfer_with_external() {
    ...
    let op_additional_in = OperationBundle {
    ...
    let op_additional_out = OperationBundle {
        create: vec![
            &e,
            (utxo_d.public_key.clone(), 200_i128),
            (utxo_e.public_key.clone(), 200_i128),
